...
<template>
  <div
    class="flex flex-col bg-primary rounded-lg *:max-w-min border-2 border-primary gap-3 p-4"
  >
    <div class="flex flex-ro gap-2 mb-0">
      <VaChip outline color="#FFF" square size="medium" readonly>
        Dispatch
      </VaChip>
      <VaInput v-model="dispatcher" placeholder="Dispatcher"></VaInput>
      <VaButton
        preset="primary"
    ...
    <div
      ...
      v-if="keys.length > 0"
    >
      <VaChip outline color="#FFF" square size="medium" readonly>Key </VaChip>
    ...
    <div class="flex flex-row ml-24 m-2 rounded-lg *:pr-4">
      <structvalue class="bg-secondary" v-model:structvalue="type" />
    </div>
  </div>
</template>
